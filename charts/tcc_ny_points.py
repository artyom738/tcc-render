...
class TccNyPoints(TccNy):
	def get_additional_stat_info(self, song: 'Song'):
		ny_chart = chart_repository.get_last_chart_by_type('tcc_ny').fill()
		...
		points_info = f'Очков в {year_for_chart}: {song.get_points("tcc", datetime.strptime(f'{year_for_chart}-01-01', "%Y-%m-%d"), datetime.strptime(f'{current_year}-01-01', "%Y-%m-%d"))}'
