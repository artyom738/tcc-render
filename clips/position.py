...
def create_position_clip(params: dict):
	...
	try:
		...
		# Явно закрываем исходные клипы для освобождения ресурсов FFmpeg
		if clip1:
			clip1.close()
		if bg_video:
			bg_video.close()
...
def create_last_out_clip(params: dict):
	...
	music_clip_source = None
	bg_video = None
	try:
		music_clip_source = mp.VideoFileClip(clip_path)
		music_clip = music_clip_source \
		...
		# Явно закрываем исходные клипы для освобождения ресурсов FFmpeg
		if music_clip_source:
			music_clip_source.close()
		if bg_video:
			bg_video.close()
